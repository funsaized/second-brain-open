...
* Remaining P2/R8 gate: actual native four-file application, applied-wiki query, repeat/interruption/recovery, edit-authorized injection and owner content acceptance. Launch guards/fake-provider errors do not establish every model-level missing-resource refusal. Synthetic privacy preparation, runs and bounded operator-validated edits are authorized; personal sources remain out of scope. Real-source/provider policy, private backup/restore, R7B private reporting and P3 installation remain gated. Namespace fixtures are ephemeral; ordinary live OpenCode sessions retain their normal local history.
