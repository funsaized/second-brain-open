...
> Status: P2 machinery and live staged rehearsal delivered; unsafe wrappers withheld; native acceptance pending
> Updated: 2026-09-24
...
* Verification: `python3 -m unittest discover -s tests -v` passes 29 offline checks (distribution, scope, rollback, contract/framework/checker and live-driver guards). `python3 tests/runtime_read_probe.py` passes 16 baseline cases on OpenCode `1.18.32` / Bubblewrap `0.12.0`; `python3 tests/runtime_roles_probe.py` passes 25 native loading/tool checks and enforces withholding the unsafe wrappers. Its separate command characterizations retain four failed safety results. Static tests are not runtime proof, and a deterministic fake provider is not model-ingestion proof.
...
* Remaining P2 gate: end-to-end runs of the actual named roles, including accepted edits and sourced answer format, source injection/interruption/recovery and missing-role/skill refusal. P2A/P2B remain required. Real-source/provider policy, private backup/restore and P3 installation remain gated; synthetic approval does not authorize private notes. Namespace fixtures are ephemeral; ordinary live OpenCode sessions retain their normal local history.
